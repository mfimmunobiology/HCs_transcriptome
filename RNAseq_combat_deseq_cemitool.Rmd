...
```{r cemitool, message = FALSE, warning = FALSE, echo=TRUE, results='hide'}
# Define gene sets path if you have it
gene_sets <- read_gmt(paste0(workingdir, "genesets/ReactomePathwaysLevel3_2021-03.gmt"))


# 6. Load the CEMiTool gene interactions file
...
for (comparison in comparisons) {
  ...
  # Split comparison string to get conditions
  conditions <- strsplit(comparison, "_vs_")[[1]]
  group1 <- conditions[1]
  group2 <- conditions[2]
  
  # Subset sample metadata for the current comparison
  sample_subset <- sample_info %>% 
    filter(Group %in% c(group1, group2))

  # colnames(sample)[colnames(sample) == "Group"] <- "Class"
  # colnames(sample_subset)[colnames(sample_subset) == "Group"] <- "Class"
  
  # Subset expression data to include only samples in the current comparison
  counts_subset <- exprs_norm_batch_corrected[, colnames(exprs_norm_batch_corrected) %in% sample_subset$SampleName]
  counts_subset <- as.data.frame(counts_subset)
  # Run CEMiTool
  cem <- cemitool(expr = counts_subset, 
                  annot = sample_subset, 
                  gmt = gene_sets, 
  ...
  cem_list[[comparison]] <- cem
  # Save results
  # Define a directory to save the results
  results_dir <- paste0(workingdir, "cemitool/", comparison)
  if (!dir.exists(results_dir)) {
    dir.create(results_dir, recursive = TRUE)
  }
  ...
  write_files(cem, directory = paste0(workingdir, "cemitool/", comparison, "/tables/"), force = TRUE)
  save_plots(cem, "all", directory = paste0(workingdir, "cemitool/", comparison, "/plots/"), force = TRUE)
  diagnostic_report(cem, directory = paste0(workingdir, "cemitool/", comparison, "/diagnostics/"), force = TRUE)
  save(cem, file = paste0(workingdir, "cemitool/", comparison, "/", comparison, ".Rdata"), compress = FALSE)
  
  # Print progress
  cat("Completed CEMiTool for comparison:", comparison, "\n")
}
```
