...
# Initialize an empty list to store individual dataframes with added comparison identifiers and gene names
df_list <- list()

# Loop over the results list
for (comparison in names(results_list)) {
  # Extract the dataframe from the list
  result_df <- as.data.frame(results_list[[comparison]])
  
  # Add a new column for the gene names (rownames) and the comparison identifier
  result_df$Gene <- rownames(result_df)
  result_df$Comparison <- comparison
  
  # Store the modified dataframe in the list
  df_list[[comparison]] <- result_df
}

# Combine all the dataframes into one
combined_df_CMV_T <- bind_rows(df_list)

# Save the combined dataframe to a CSV file
write.csv(combined_df_CMV_T, "/home/vivischuch/Documents/science_immunology_application/DEGsForCMV_TvsNT_T.csv", row.names = FALSE)
```
...
png(paste0(outputfolder, "meta_degs_vote@MetaVolcano_", jobname, ".png"))
meta_degs_vote@MetaVolcano
dev.off()

svg_file_path <- paste0(outputfolder, "meta_degs_vote@MetaVolcano_", jobname, ".svg")
svg(svg_file_path)
meta_degs_vote@MetaVolcano
dev.off()

png(paste0(outputfolder, "meta_degs_vote@degfreq_", jobname, ".png"))
meta_degs_vote@degfreq
dev.off()

svg_file_path <- paste0(outputfolder, "meta_degs_vote@degfreq_", jobname, ".svg")
svg(svg_file_path)
meta_degs_vote@degfreq
dev.off()
...
gsea_sig_filtered$leadingEdge <- sapply(gsea_sig_filtered$leadingEdge, function(x) paste(x, collapse = ", "))

write.csv(gsea_sig_filtered, paste0("/home/vivischuch/Documents/science_immunology_application/Supplemenmtaty_gsea_sig_metaDegs_hallmark_", jobname, ".csv"),row.names=F)

gsea_sig_filtered_CMV_T <- gsea_sig_filtered
```
...
# Combine all the dataframes into one
combined_df_CMV_EG <- bind_rows(df_list)

# Save the combined dataframe to a CSV file
write.csv(combined_df_CMV_EG, "/home/vivischuch/Documents/science_immunology_application/DEGsForCMV_EGvsNT_EG.csv", row.names = FALSE)
```
...
gsea_sig_filtered_CMV_EG <- gsea_sig_filtered
```
...
# Combine all the dataframes into one
combined_df_HIV_T <- bind_rows(df_list)

# Save the combined dataframe to a CSV file
write.csv(combined_df_HIV_T, "/home/vivischuch/Documents/science_immunology_application/DEGsForHIV_TvsNT_T.csv", row.names = FALSE)
```
...
gsea_sig_filtered_HIV_T <- gsea_sig_filtered
```
...
# Combine all the dataframes into one
combined_df_HIV_EG <- bind_rows(df_list)

# Save the combined dataframe to a CSV file
write.csv(combined_df_HIV_EG, "/home/vivischuch/Documents/science_immunology_application/DEGsForHIV_EGvsNT_EG.csv", row.names = FALSE)
```
...
gsea_sig_filtered_HIV_EG <- gsea_sig_filtered

```

```{r}
# Add Virus and Gestation Columns to each GSEA result table
gsea_sig_filtered_CMV_T$Virus <- "CMV"
gsea_sig_filtered_CMV_T$Gestation <- "Term"

gsea_sig_filtered_HIV_T$Virus <- "HIV"
gsea_sig_filtered_HIV_T$Gestation <- "Term"

gsea_sig_filtered_CMV_EG$Virus <- "CMV"
gsea_sig_filtered_CMV_EG$Gestation <- "Early Gestation"

gsea_sig_filtered_HIV_EG$Virus <- "HIV"
gsea_sig_filtered_HIV_EG$Gestation <- "Early Gestation"

# Combine all the tables into one
combined_gsea_sig <- rbind(
  gsea_sig_filtered_CMV_T,
  gsea_sig_filtered_HIV_T,
  gsea_sig_filtered_CMV_EG,
  gsea_sig_filtered_HIV_EG
)

# Check the combined data
head(combined_gsea_sig)

# Save the combined table as a CSV file for the supplementary table
write.csv(combined_gsea_sig, "/home/vivischuch/Documents/science_immunology_application/Supplemenmtaty_combined_gsea_sig_results.csv", row.names = FALSE)
```

```{r}
# Direct comparison of column names
identical(colnames(combined_df_CMV_EG), colnames(combined_df_CMV_T)) 
identical(colnames(combined_df_CMV_EG), colnames(combined_df_HIV_EG)) 
identical(colnames(combined_df_CMV_EG), colnames(combined_df_HIV_T)) 
...
combined_results <- combined_results %>%
  mutate(
    Virus = case_when(
      grepl("CMV", Comparison) ~ "CMV",
      grepl("HIV", Comparison) ~ "HIV",
      TRUE ~ "Unknown"
    ),
    Gestation_Time = case_when(
      grepl("_eg", Comparison) ~ "Early/mid",
      grepl("_T", Comparison) ~ "Term",
      TRUE ~ "Unknown"
    )
  )
...
write.csv(combined_results, "/home/vivischuch/Documents/science_immunology_application/Supplementary_Table_Differential_Expression.csv", row.names = FALSE)
...
# Set file paths
files_rem <- list.files(path = "/home/vivischuch/Documents/RNAseq_inhouse_data/metavolcano/", full.names = TRUE, pattern = "rem@metaresult_.*\\.csv$")
files_vote <- list.files(path = "/home/vivischuch/Documents/RNAseq_inhouse_data/metavolcano/", full.names = TRUE, pattern = "vote@metaresult_.*\\.csv$")

# Function to extract virus and gestation from filename
extract_info <- function(filepath) {
  filename <- basename(filepath)
  virus <- ifelse(grepl("HIV", filename), "HIV", "CMV")
  gestation <- ifelse(grepl("_T", filename), "Term", "Early/mid")
  return(list(virus = virus, gestation = gestation))
}

# Read all "rem" files and add virus/gestation columns
rem_combined <- bind_rows(lapply(files_rem, function(file) {
  data <- read.csv(file)
  info <- extract_info(file)
  data$virus <- info$virus
  data$gestation <- info$gestation
  return(data)
}))

# Read all "vote" files and add virus/gestation columns
vote_combined <- bind_rows(lapply(files_vote, function(file) {
  data <- read.csv(file)
  info <- extract_info(file)
  data$virus <- info$virus
  data$gestation <- info$gestation
...
write.csv(rem_combined, "/home/vivischuch/Documents/science_immunology_application/Supplementary_Table_combined_rem_metadegs.csv", row.names = FALSE)
write.csv(vote_combined, "/home/vivischuch/Documents/science_immunology_application/Supplementary_Table_combined_vote_metadegs.csv", row.names = FALSE)
...
write.csv(merged_data, file = "/home/vivischuch/Documents/science_immunology_application/Supplementary_Table_DEGs_placenta_genes.csv", row.names = FALSE)
